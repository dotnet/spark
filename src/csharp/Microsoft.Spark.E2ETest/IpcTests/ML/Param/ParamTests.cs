...
namespace Microsoft.Spark.E2ETest.IpcTests.ML.ParamTests
{
    ...
    public class ParamTests
    {
        ...
        [InlineData(false)]
        [InlineData(true)]
        public void Test(bool useIdentifiableParent)
        {
            const string expectedParent = "parent";
            const string expectedName = "name";
            const string expectedDoc = "doc";

            var param = useIdentifiableParent
                ? new Param(new Tokenizer(expectedParent), expectedName, expectedDoc)
                : new Param(expectedParent, expectedName, expectedDoc);
