    ...
    try {
      ...
      case e: Throwable =>
        ...
        val argsStr = args
          ...
          .mkString(", ")
        val methodType = if (isStatic) {
          "static method"
        } else {
          "method"
        }
        logError(s"Failed to execute $methodType '$methodName' on '$jvmObjName' with " +
          s" args=($argsStr)")
        ...
        serDe.writeInt(dos, -1)
        serDe.writeString(dos, Utils.exceptionString(if (e.getCause != null) e.getCause else e))
    }
  }
