...
class TpchFunctionalQueries(spark: SparkSession, tpchRoot: String)
  ...
  def q1(): Unit = {
    val decrease = udf { (x: Double, y: Double) =>
      x * (1 - y)
    }
    val increase = udf { (x: Double, y: Double) =>
      x * (1 + y)
    }
  ...
  def q2(): Unit = {
    ...
    val brass = part
      .filter(part("p_size") === 15 && part("p_type").endsWith("BRASS"))
  ...
  def q8(): Unit = {
    ...
    val isBrazil = udf { (x: String, y: Double) =>
      if (x == "BRAZIL") y else 0
    }
  ...
  def q9(): Unit = {
    ...
    val expr = udf { (x: Double, y: Double, v: Double, w: Double) =>
      x * (1 - y) - (v * w)
    }
  ...
  def q11(): Unit = {
    val mul = udf { (x: Double, y: Int) =>
      x * y
    }
    val mul01 = udf { x: Double =>
      x * 0.0001
    }
  ...
  def q14(): Unit = {
    val reduce = udf { (x: Double, y: Double) =>
      x * (1 - y)
    }
  ...
  def q16(): Unit = {
    val complains = udf { x: String =>
      x.matches(".*Customer.*Complaints.*")
    }
    val polished = udf { x: String =>
      x.startsWith("MEDIUM POLISHED")
    }
    val numbers = udf { x: Int =>
      x.toString.matches("49|14|23|45|19|3|36|9")
    }

    val fparts = part
      .filter(
        ($"p_brand" =!= "Brand#45") && !polished($"p_type") &&
          numbers($"p_size"))
  ...
  def q19(): Unit = {
    val sm = udf { x: String =>
      x.matches("SM CASE|SM BOX|SM PACK|SM PKG")
    }
    val md = udf { x: String =>
      x.matches("MED BAG|MED BOX|MED PKG|MED PACK")
    }
    val lg = udf { x: String =>
      x.matches("LG CASE|LG BOX|LG PACK|LG PKG")
    }
  ...
  def q20(): Unit = {
    ...
    part
      ...
      .join(
        flineitem,
        $"ps_suppkey" === flineitem("l_suppkey") && $"ps_partkey" === flineitem("l_partkey"))
  ...
  def q21(): Unit = {
    ...
    val plineitem =
      lineitem.select($"l_suppkey", $"l_orderkey", $"l_receiptdate", $"l_commitdate")
  ...
  def q22(): Unit = {
    val sub2 = udf { x: String =>
      x.substring(0, 2)
    }
    val phone = udf { x: String =>
      x.matches("13|31|23|29|30|18|17")
    }

    val fcustomer = customer
